...
Multiple Correspondence Analysis (MCA) is a method to analyze qualitative data and it is an extension of Correspondence analysis (CA). MCA can be used to detect patterns or structure in the data as well as in dimension reduction. In other words it tries to pack information from the data to some categorical variables, so that we can in a way group individual observations. 

Tea-dataset is a data frame with 300 rows and 36 columns. Rows represent the individuals, columns represent the different questions. There was quite a lot ready-made and handy R-code for plots in the web and I used them in analysis for MCA. 
...
Let's first apply MCA to the whole tea-data. For that we use code that was found in the web for this tea-data analysis.
...
This week was really fun and I took quite a bit of liberties doing the exercises. Of course understanding is limited for methods, but there was good material in the web. 